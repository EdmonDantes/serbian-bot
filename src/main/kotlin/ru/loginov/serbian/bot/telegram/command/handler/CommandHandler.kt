package ru.loginov.serbian.bot.telegram.command.handler
...
import ru.loginov.serbian.bot.configuration.TelegramCallbackExecutor
...
class CommandHandler(
        ...
        @Qualifier("telegram_callback_executor") private val callbackExecutor: TelegramCallbackExecutor,
    ...
    override suspend fun onUpdate(update: Update) {
        ...
        val (first, second) = coroutineScope {
            update.message?.let { async(dispatcher) { processUpdateMessage(update) } } to
                    update.callbackQuery?.let { async(dispatcher) { processCallbackQuery(update) } }
        }
    ...
    private suspend fun processUpdateMessage(update: Update) {
        ...
        if (!update.message!!.text.startsWith('/')) {
            val data = CallbackData(data = update.message?.text, location = update.message?.location)
            callbackExecutor.invoke(chatId to userId, data)
            return
        }
        ...
        callbackExecutor.cancel(chatId to userId)
    ...
    private suspend fun processCallbackQuery(update: Update) {
        ...
        callbackExecutor.invoke(chatId to userId, CallbackData(data = dataStr))
    }
